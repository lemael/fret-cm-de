...
export default function ShippedHistoryScreen() {
  ...
  const fetchParcels = useCallback(async () => {
    try {
      const res = await shipmentsAPI.shippedHistory();
      setParcels(res.data);
    } catch {
      setParcels([]);
    } finally {
  ...
  return (
    ...
    >
      <Text style={styles.title}>Historique d'envoi de colis</Text>
      <Text style={styles.subtitle}>
        Colis entièrement chargés, confirmés et clôturés.
      </Text>
      ...
      <View style={styles.table}>
        ...
        {loading ? (
          ...
          <Text style={styles.emptyText}>Aucun colis envoyé pour le moment.</Text>
        ) : (
          parcels.map((parcel) => (
            <TouchableOpacity
              key={parcel.id}
            ...
            >
              ...
              <Text style={[styles.cell, styles.colDate]}>{formatDate(parcel.updated_at)}</Text>
            </TouchableOpacity>
          ))
        )}
      </View>
